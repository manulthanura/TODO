...
class User {
    ...
    public function __construct($name, $email, $password) {
      ...
      $this->regdate = date("Y-m-d H:i:s"); // Set registration date to current date and time
    }
  
    // Method to save user to database
    public function save($conn) {
      $sql = "INSERT INTO users (name, email, password, regdate) VALUES (?, ?, ?, ?)";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("ssss", $this->name, $this->email, $this->password, $this->regdate);
      $stmt->execute();
